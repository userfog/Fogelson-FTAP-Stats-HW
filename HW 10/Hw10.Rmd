...
oneDayVar <- .3^2*(beta.bestBuy^2*forecast$standardDeviation^2 +  var(lm.bestBuy$residuals)) 
  ...
  cov[length(bestBuy$excessReturn)]*.3*.7
oneDayVar
```
...
$$\frac{d^2\mathcal{L}}{d\omega d\alpha} = \Sigma_2^N(\frac{x_{i_1}}{(\omega + \alpha * x_{i-1})^2} - 2* \frac{x_{i-1}^2}{(\omega + \alpha * x_{i-1})^3})$$
...
\Sigma_2^N(\frac{x_{i_1}}{(\omega + \alpha * x_{i-1})^2} - 2* \frac{x_{i-1}^2}{(\omega + \alpha * x_{i-1})^3}) & \Sigma_2^N(\frac{1}{(\omega + \alpha * x_{i-1})^2} - 2* \frac{x_{i-1}}{(\omega + \alpha * x_{i-1})^3})
